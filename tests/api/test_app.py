...
def mock_app(mock_test_data):
    ...
    with patch(
        "zpodcast.parsers.json.PodcastJSON.import_podcast_list"
    ) as mock_import_list, patch(
        "zpodcast.parsers.json.PodcastJSON.import_podcast_playlist"
    ) as mock_import_playlist:
...
def mock_file_content():
    ...
    podcast_playlist_content = {
        "version": PodcastJSON.VERSION,
        "podcastplaylist": {"playlists": [{"name": "Test Playlist", "episodes": []}]},
    }
...
def test_get_podcasts(test_client):
    ...
    with patch(
        "zpodcast.core.podcasts.PodcastList.to_dict", return_value={"podcasts": []}
    ):
...
def test_get_playlists(test_client):
    ...
    with patch(
        "zpodcast.core.playlists.PodcastPlaylist.to_dict",
