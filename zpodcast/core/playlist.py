...
class PodcastEpisodeList:
    ...
    def __init__(self, name: str, episodes: List[PodcastEpisode]):
        self.name = name
    ...
    def _validate_name(self, name: str) -> None:
        if not isinstance(name, str):
            raise ValueError(
                f"Invalid playlist name. Playlist name is not a string. {name}"
            )

        if not bool(re.match(r"([a-zA-Z0-9 ]+$)", name)):
            raise ValueError(
                f"Invalid playlist name. The name must be alphanumeric. Name = {name}"
            )

        if len(name) > 100:
            raise ValueError(
                f"Invalid playlist name. The name is {len(name)} and the maximum length is 100 characters."
            )
    ...
    def move_episode_up(self, index: int) -> None:
        if index > 0 and index < len(self._episodes):
            self._episodes[index], self._episodes[index - 1] = (
    ...
    def get_episodes(self, indices: List[int] = None) -> List[PodcastEpisode]:
        ...
        valid_indices = [
            i for i in indices if isinstance(i, int) and 0 <= i < len(self._episodes)
        ]
